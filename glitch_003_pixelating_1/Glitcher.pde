class Glitcher extends PImage {
  ...
  PImage offset(int x, int y, int w, int h, int randAmount) {
    ...
    //int rw = int(random(100));
    //int rh = int(random(100));
    //this.pg.beginDraw(); 
    PGraphics ppg = createGraphics(this.img.width, this.img.height);
    ppg.beginDraw();

    this.img = this.pg.copy();    ppg..pg image(this.img, 0, 0);
    ppg.copy(this.img,x,y,w,h,int(x+random(randAmount)),int(y+random(randAmount)),w,h);
    image(ppg, 0, 0);
    ...
    //this.pg.image(this.img, 0, 0);
    //this.pg.copy(this.img,x,y,w,h,int(x+random(randAmount)),int(y+random(randAmount)),w,h);
    //this.img = this.pg.copy();
    //this.pg.endDraw();
    //return this;
  }
